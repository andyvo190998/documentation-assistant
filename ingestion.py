...
async def index_documents_async(documents: List[Document], batch_size: int = 50):
    ...
    if successful == len(batches):
        log_success(f"VectorStore Indexing: Successfully added {successful}/{len(batches)} documents")
    else:
        log_warning(f"VectorStore Indexing: Failed to add {successful}/{len(batches)} documents")
...
async def main():
    ...
    # Process documents asyn
    await index_documents_async(splitted_docs, 500)
